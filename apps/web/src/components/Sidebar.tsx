...
import { ClaudeAI, OpenAI, T3Wordmark } from "./Icons";
...
function SidebarThreadRow(props: SidebarThreadRowProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div title={thread.title} className="flex min-w-0 flex-1 items-center gap-1.5 text-left">
          {prStatus && (
            <Tooltip>
              <TooltipTrigger
                render={
...
function SidebarT3Wordmark() {
  return <T3Wordmark className="h-2.5 w-auto shrink-0 text-foreground" />;
}
    ...
    return (
      <>
        <div className="group/project-header relative">
          ...
          >
            {project.expanded ? (
              <FolderOpenIcon className="size-3.5 shrink-0 text-muted-foreground/80" />
            ) : (
              <FolderClosed className="size-3.5 shrink-0 text-muted-foreground/80" />
            )}
        ...
        <SidebarMenuSub
          ...
          className="my-1 w-full translate-x-0 gap-0.5 overflow-hidden px-2.5 py-0"
        >
