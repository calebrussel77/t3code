...
interface CustomSpinnerProps extends Omit<
...
export const CustomSpinner = React.forwardRef<
...
>(({ className, size = "md", speed = "normal", ...props }, ref) => {
  return (
    <span
      data-spinner
      className={cn("inline-block shrink-0", speedClasses[speed], sizeClasses[size], className)}
    >
      <SpinnerPrimitive
        ref={ref}
        aria-hidden
