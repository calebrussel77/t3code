...
type ComposerSegment =
  ...
  | { type: "skill-mention"; skillName: string }
  | { type: "terminal-context"; context: unknown };

const SLASH_COMMANDS: readonly ComposerSlashCommand[] = ["model", "plan", "default"];
const isInlineTokenSegment = (segment: ComposerSegment): boolean => segment.type !== "text";
...
function collapsedSegmentLength(segment: ComposerSegment): number {
...
function clampCollapsedComposerCursorForSegments(
  segments: ReadonlyArray<ComposerSegment>,
  cursorInput: number,
): number {
